...
def predict(params, var, test_years, lead_years,model_year = None,  results_dir=None, num_stds = 1 , truncated = False):
    ...
    if ensemble_list is not None: ## PG: calculate the mean if ensemble mean is none
        ...
        ds_in = xr.open_dataset(data_dir_forecast).sel(ensembles = ensemble_list).load()[var]
        ...
        print("Load forecasts") 
        ds_in = xr.open_dataset(data_dir_forecast).mean('ensembles').load()[var]
                        ...
                        if params['output_sampling'] == 'conditional_multinormal_based_on_train_sampling':
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    residual_covs[month] += torch.eye(residual_covs[month].shape[0]) * 1e-5
                                except RuntimeError:
        ...
        for i, (x, target) in enumerate(test_set):          
                ...
                with torch.no_grad():
                    ...
                    if params['output_sampling'] in ['Gaussian_noise', 'normal_based_on_train_sampling']:
                        ...
                        epsilon =  torch_normal_sampling(torch.zeros(out.shape[-1]), residual_std, num_samples=  out.shape[0],truncated_dist = truncated_dist ).to(device).squeeze()
                        ...
                        epsilon = torch_normal_sampling(torch.zeros(out.shape[-1]), residual_cov, num_samples=  out.shape[0], multivariate=True ).to(device).squeeze()
...
if __name__ == "__main__":
    ...
    var = 'tas'
    fake_data = 'pi'
    out_dir_x  = f'/space/hall5/sitestore/eccc/crd/ccrn/users/rpg002/output/{var}/SOM-FFN/results/Autoencoder/run_set_final_picontrol'
    out_dir    = f'{out_dir_x}/N1_v2_Banealing_L0_archNone_batch100_e150_BVAE_50-100_picontrol_TSE5_LS500_beta0.5' 
    ...
    params['non_random_decoder_initialization'] =  'normal_based_on_train_sampling'## False,'normal_based_on_train_sampling' , True ('condition_dependent_latent'), 'encoder_based_sampling', 'histogram_based_sampling', 'normal_based_PCA_train_sampling
    params['BVAE'] = 1000
    ...
    predict(params, var, test_years=test_years,model_year = 49, num_stds = num_stds, truncated = truncated , lead_years=lead_years,  results_dir=out_dir)
