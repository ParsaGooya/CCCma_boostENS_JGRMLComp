...
from data_locations import LOC_FORECASTS_fgco2_normal_2pi, LOC_FORECASTS_fgco2_normal_pi, LOC_FORECASTS_fgco2_normal, LOC_OBSERVATIONS_fgco2_v2023, LOC_FORECASTS_fgco2
import gc
import glob
...
def predict(params, test_years, lead_years,model_year = None,  results_dir=None, num_stds = 1 , truncated = False):
    ...
    if params['non_random_decoder_initialization'] not in [False, 'normal_based_on_train_sampling', 'ensemble_mean_based_normal_sampling']:
        truncated = False
    ...
    if 'latentdependant' in  results_dir:
        ...
        assert params['non_random_decoder_initialization'] not in ['histogram_based_sampling', 'normal_based_on_train_sampling' , 'ensemble_mean_based_normal_sampling']
        params['full_conditioning'] = True
        if params['prior_flow'] is not None:
    ...
    if 'condition_type' not in params.keys():
        params['condition_type'] = 'ensembles_mean'
    ...
    for y_idx, test_year in enumerate(test_years):
        ...
        if params['remove_ensemble_mean']:  
            ...
            if params['version'] == 2:
                ...
                ds_std_test =  ds_em_std.sel(year = slice(test_year, test_year))
                del  ds_em_std
        else:
        ...
        if any([params['non_random_decoder_initialization'] in ['histogram_based_sampling','normal_based_on_train_sampling'], truncated, params['output_sampling'] in ['normal_based_on_train_sampling', 'multinormal_based_on_train_sampling']]):
        ...
        if params['time_features'] is not None:
            if any(['sin_t' in time_features, 'cos_t' in time_features, 'y' in time_features]):
                max_month = len(train_years) * 12
            else:
                max_month = None
        else:
                max_month = None
        ...
        test_set = XArrayDataset(ds_test, obs_test, lead_time = lead_time, extra_predictors= extra_predictors,lead_time_mask = params['lead_time_mask'], time_features=time_features,  in_memory=False, aligned = True, year_max = year_max, max_month = max_month, BVAE = params['BVAE'])
        ...
        if any([params['non_random_decoder_initialization'] in ['histogram_based_sampling','normal_based_on_train_sampling'],truncated, params['output_sampling'] in ['normal_based_on_train_sampling', 'multinormal_based_on_train_sampling']]):
            ...
            with torch.no_grad():
                ...
                if (type(x_in) == list) or (type(x_in) == tuple):
                    x_in = x_in[0]
            ...
            if params['non_random_decoder_initialization'] == 'normal_based_on_train_sampling':
                ...
                hist, edges = np.histogramdd(train_samples, bins= 200 ) 

            if params['output_sampling'] in ['normal_based_on_train_sampling', 'multinormal_based_on_train_sampling']:

                x_residuals = torch.flatten(x_residuals, start_dim = 0, end_dim = -2)
        
                if params['output_sampling'] == 'multinormal_based_on_train_sampling':
                    ...
                    try:
                        ...
                        try:
                            ...
                            residual_covs += torch.eye(residual_covs.shape[0]) * 1e-5
                        except RuntimeError:
                            print("Covariance matrix is not positive definite!")
                else:
                    residual_covs = torch.std(x_residuals, dim = 0)

            del  train_samples, x_residuals
        ...
        for i, (x, target) in enumerate(test_set):          
                ...
                with torch.no_grad():
                    ...
                    if params['non_random_decoder_initialization'] is False:
                        ...
                        elif params['non_random_decoder_initialization'] == 'ensemble_mean_based_normal_sampling':
                            if (type(x) == list) or (type(x) == tuple):
                                x_in = (test_raw[0].mean(0).unsqueeze(0), test_raw[1])
                            else:
                                x_in = test_raw.mean(0).unsqueeze(0)
                            _, mu, _ = net(x_in, condition = cond, sample_size = 1)[:3]
                            z = torch_normal_sampling( - 1 *mu.squeeze(), num_stds *  torch.ones(( net.latent_size)), num_samples=  params['BVAE']* sample_size,truncated_dist = truncated_dist ).to(device)
                            del x_in
                        else:   
                    ...
                    #########
                    out = net.decoder(z)
                    if params['output_sampling'] in ['Gaussian_noise', 'normal_based_on_train_sampling']:
                        stds = torch.ones(out.shape[-1]) if params['output_sampling'] == 'Gaussian_noise' else residual_covs
                        epsilon =  torch_normal_sampling(torch.zeros(out.shape[-1]), stds, num_samples=  out.shape[0],truncated_dist = truncated_dist ).to(device).squeeze()
                                                  
                    elif params['output_sampling'] == 'multinormal_based_on_train_sampling':
                        epsilon = torch_normal_sampling(torch.zeros(out.shape[-1]), residual_covs, num_samples=  out.shape[0], multivariate=True ).to(device).squeeze()
                                     
                    elif params['output_sampling'] == 'modeled_varaince':
                        NotImplementedError('Not implemented yet!')
                    
                    else:
                        epsilon = torch.zeros(out.shape)
        ...
        out_name = f'tests2/nn_adjusted_{test_year}_saved_model_'
...
if __name__ == "__main__":
    ...
    out_dir_x  = f'/space/hall5/sitestore/eccc/crd/ccrn/users/rpg002/output/fgco2_ems/SOM-FFN/results/Autoencoder/run_set_final_toy'
    out_dir    = f'{out_dir_x}/N2_v3_Banealing_L0_archNone_batch100_e50_cEBVAE_10-1_NnRandDecodInit_hist_fake_data_normal_pi_beta0.1_weightedMSE_TSE20' 

    if '_pi' in out_dir:
        fake_data = 'pi'
    elif '_2pi' in out_dir:
        fake_data = '2pi'
    else:
        fake_data = '1'
    ...
    ### handles
    num_stds = 1.05 ### params['snon_random_decoder_initialization'] should be False or 'normal_based_on_train_sampling'
    truncated = False ### params['non_random_decoder_initialization'] should be False or 'normal_based_on_train_sampling'
    params['non_random_decoder_initialization'] =  'normal_based_on_train_sampling' ## False,'normal_based_on_train_sampling' , True ('condition_dependent_latent'), 'encoder_based_sampling', 'histogram_based_sampling', 'ensemble_mean_based_normal_sampling
    params['BVAE'] = 100
    params['output_sampling'] = None  # None, Gaussian_noise, normal_based_on_train_sampling, multinormal_based_on_train_sampling,  modeled_varaince, 
    test_years = np.arange(2010,2015) #
    

    Path(out_dir + '/tests2').mkdir(parents=True, exist_ok=True)

    if fake_data is not None:
        if fake_data == '2pi':
            data_dir_forecast = LOC_FORECASTS_fgco2_normal_2pi
        elif fake_data == 'pi':
            data_dir_forecast = LOC_FORECASTS_fgco2_normal_pi
        else :
            data_dir_forecast = LOC_FORECASTS_fgco2_normal
        unit_change = 1

    predict(params, test_years=test_years,model_year = 2010, num_stds = num_stds, truncated = truncated , lead_years=lead_years,  results_dir=out_dir)
