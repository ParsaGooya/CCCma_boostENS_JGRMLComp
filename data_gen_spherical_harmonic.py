...
def sample_gen(time, freq_coeffs, harmonic_wavenumbers, harmonic_m, pi_delta = False):
    ...
    for t in time:
        if pi_delta:
            ...
            sin_phase =  np.sin((t * 2 * np.pi / freq_coeffs) +  s)
...
# ds_in = xr.open_mfdataset(str(Path(LOC_FORECASTS_fgco2, "*.nc")), combine='nested', concat_dim='year').mean('ensembles').load()['fgco2'].sel(year = slice(1965,None))
# time = np.arange(len(ds_in.year) * 12)
# years = ds_in.year.values
# del ds_in
# ls = []
# for member in tqdm(range(num_samples)):
#     ds = sample_gen(time, freq_coeffs, harmonic_wavenumbers, harmonic_m, pi_delta = pi_delta) 
...
# name_out = 'fake_normal_historical_ensembles_196501-201412_1x1_LE'
# if pi_delta:
#   name_out =  name_out + '_pi'
# fake_ds.to_netcdf(LOC_FORECASTS_fgco2 + f'/fake_test/{name_out}.nc')

############################################################################ large target dataset ############################################################################

ds_fake_path = '/space/hall5/sitestore/eccc/crd/ccrn/users/rpg002/data/fgco2/simulation/fake_test/fake_normal_historical_ensembles_196501-201412_1x1_LE.nc'

ds_fake = xr.open_dataset(ds_fake_path)['fgco2']

if '_pi' in ds_fake_path:
    pi_delta = True
else:
    pi_delta = False
...
ds_in = xr.open_mfdataset(str(Path(LOC_FORECASTS_fgco2, "*.nc")), combine='nested', concat_dim='year').mean('ensembles').load()['fgco2'].sel(year = ds_fake.year)
...
fake_ds.attrs['Spherical_harmonic_order'] = harmonic_m
fake_ds.attrs['units'] = 'month, deg, km'
name_out = f'fake_normal_historical_ensembles_2010-2014_1x1_{num_samples}LE'
if pi_delta:
    name_out = name_out + '_pi'

fake_ds.assign_coords(ensembles = np.arange(num_samples)).to_netcdf(LOC_FORECASTS_fgco2 + f'/fake_test/{name_out}.nc')
